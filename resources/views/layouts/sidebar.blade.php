...
<div class="flex items-center justify-between flex-shrink-0 px-4 py-4 border-b border-slate-700">
    ...
    <!-- Tombol Tutup untuk mobile, terlihat di bawah breakpoint lg -->
...
{{-- Navigasi Menu --}}
<nav class="mt-5 flex-grow px-2 space-y-1 overflow-y-auto">
    {{-- Variabel $dynamicMenuItems sekarang berasal dari MenuServiceProvider --}}
    @foreach($dynamicMenuItems as $group => $menus)
        ...
        @foreach($menus as $menu)
            {{-- Menggunakan relasi 'children' yang di-eager load oleh provider --}}
            @if($menu->children->isEmpty())
                {{-- Menu Tanpa Submenu --}}
                <a href="{{ route($menu->route) }}"
                   class="{{ request()->routeIs($menu->route.'*') ? 'bg-slate-900 text-white' : 'text-slate-300 hover:bg-slate-700 hover:text-white' }} group flex items-center px-2 py-2 text-sm font-medium rounded-md">
                    {!! $menu->icon_svg !!}
            ...
            @else
                ...
                <div x-data="{ open: {{ request()->routeIs($menu->route.'*') ? 'true' : 'false' }} }">
                    <button @click="open = !open"
                            class="w-full {{ request()->routeIs($menu->route.'*') ? 'bg-slate-900 text-white' : 'text-slate-300 hover:bg-slate-700 hover:text-white' }} group flex items-center px-2 py-2 text-sm font-medium rounded-md justify-between">
                    ...
                    <div x-show="open" x-transition class="mt-1 space-y-1 pl-9">
                        @foreach($menu->children as $submenu)
                            <a href="{{ route($submenu->route) }}"
                               class="{{ request()->routeIs($submenu->route.'*') ? 'text-white' : 'text-slate-400 hover:text-white' }} group flex items-center py-2 text-sm font-medium rounded-md">
                                {{ $submenu->name }}
                            </a>
                        @endforeach
                    </div>
